...
void removeAtDynArr(DynArr *v, int idx)
{
	...
	if (idx < (v->size - 1 ))
	{
		...
		// move all the following elements back one
...
int isEmptyDynArr(DynArr *v)
{
	...
	if (v->size <= 0)
	{
		return 1; // for True, it's empty
	}
	else
	{
		return 0; // for False, it's not empty
	}
}
...
void printDynArr(struct DynArr * myArr)
{
	...
	for (i=0; i< sizeDynArr(myArr); i++)
	{
		printf("%d, ", getDynArr(myArr, i));
	}
	printf("\n");
}
