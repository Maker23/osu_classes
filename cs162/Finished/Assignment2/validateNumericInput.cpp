...
/***************************************************************************** 
 * TODO: refactor the first definition of getIntegerInput.
 *       This is the original function and it's a mess.
 *
 *       SEE http://www.cplusplus.com/forum/beginner/79055/  
 *
 *       For a great, simple way to implement all this
 *****************************************************************************/

/* ***************************************************************************
 *  getIntegerInput (MinValue, MaxValue, MaxNumberOfTries, Prompt);
 *
...
bool getIntegerInput(std::vector <int> *intVector, int expectedSize, int minInt, int maxInt,std::string Prompt)
{
	...
	{
		if  (inputString[inputLetter] == SPACE || inputString[inputLetter] == COM )
		{
			if (wordLetter != 0)
				wordCount++;
			wordLetter=0;
...
double validateFloatInput(char *inputString, float MinValue, float MaxValue)
{
	...
	if ( ! (inputStream  >> inputDouble)) 
	{
		if (true) {
			std::cout << "Line 397 Bad value in validateFloat " << std::endl <<
				"inputString = " << inputString << std::endl <<
				"inputDouble = " << inputDouble << std::endl;
		}
		//return Bad
		return(MinValue-1);
	}
	inputFloat = static_cast<float>(inputDouble);
	/*
	 * if ( inputDouble != inputFloat )
	{
		//complain
		if (true) {
			std::cout << "Line 409 Bad value in validateFloat " << std::endl <<
				"inputString = " << inputString << std::endl <<
				"inputDouble = " << inputDouble << std::endl <<
				"inputFloat = " << inputFloat << std::endl;
		}
		return(MinValue-1);
	}
	*/
	if ( (inputFloat < MinValue ) || (inputFloat > MaxValue) ) 
	{
 			if ( true ) {std::cout << "Line 407 Numeric value = " << inputFloat << std::endl;
   		std::cout << "ERROR: Value " << inputFloat << " is outside permitted range ["
				<< MinValue << "-" << MaxValue << "]" << std::endl;
			}
			return(MinValue-1);
	}
	...
	{
			if (true) std::cout << "Returning float " << inputFloat << std::endl;
			// TODO: set precision here or later, if an integer was passed
			return inputFloat * 1.0;
	}
}
